...
| `simulate/` | **"Simulate a Payment"** — an interactive D3 network diagram of IMF/World Bank/BIS/Treasury/Fed/central banks/SWIFT/correspondent banks/RTGS systems, with a 5-step simulation of a real cross-border payment (animated flowing-ribbon path, card-style nodes). The site's first "larger-screen visualization" (`DESIGN.md` §21's future-direction item) — see `assets/js/simulate.js` |
...
Plain static HTML/CSS/vanilla JS, no build step — one deliberate exception: `simulate/` loads
D3.js from jsdelivr (`assets/js/simulate.js`) as a plain `<script>` tag, still no bundler.
Edit, commit, push to `main`; GitHub Pages deploys automatically.
